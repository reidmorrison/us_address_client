...
module USAddressClient
  class Parser
    def self.parse_verify_response(response)
      ...
      message = "Invalid data returned from Address Service. #{exc.class.name}: #{exc.message}"
      USAddressClient.logger.error(message: message, metric: 'Supplier/USAddressClient/exception', exception: exc, payload: hash)
    ...
    def self.parse_version_response(response)
      ...
      message = "Invalid data returned from Address Service#verify. #{exc.class.name}: #{exc.message}"
      USAddressClient.logger.error(message: message, metric: 'Supplier/USAddressClient/exception', exception: exc, payload: hash)
      raise(ServiceError, message)
    end
  end
